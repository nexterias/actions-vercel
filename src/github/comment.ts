...
const createCommentBody = (payload: VercelDeploymentResult & { projectName: string }) => `
| Name | Status | Preview | Updated (UTC) |
|---|---|---|---|
| ${payload.projectName} | ${payload.status} | ${
  payload.status === "Ready" ? `[Visit Preview](${payload.deploymentUrl})` : "Not available"
...
export class VercelDeploymentIssueComment {
  ...
  public async update(payload: VercelDeploymentResult): Promise<void> {
    await this.octokit.rest.issues.updateComment({
      ...githubContext.repo,
      comment_id: this.id,
      body: [commentHeader, createCommentBody({ ...payload, projectName: this.projectName })].join(
        "\n",
      ),
    });
  }
...
export const upsertIssueComment = async (
  ...
  for await (const response of requests) {
    for (const comment of response.data) {
      if (comment.body?.includes(commentHeader)) {
        const obj = new VercelDeploymentIssueComment(octokit, comment.id, payload.projectName);
  ...
  return new VercelDeploymentIssueComment(octokit, comment.data.id, payload.projectName);
};
