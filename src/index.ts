...
import { type VercelDeploymentIssueComment, upsertIssueComment } from "./github/comment";
...
async function run() {
  ...
  if (octokit) {
    const [settledResultComment, settledCommitStatus, settledDeployment] = await Promise.allSettled(
      [
    ...
    if (settledResultComment.status === "fulfilled") resultComment = settledResultComment.value;
    else if (github.context.payload.pull_request) {
      core.warning('May need "GITHUB_TOKEN" with "write" permission to "pull_requests"');

      if (isDebug) core.error(settledResultComment.reason);
    }

    if (settledCommitStatus.status === "fulfilled") commitStatus = settledCommitStatus.value;
    else {
      core.warning('May need "GITHUB_TOKEN" with "write" permission to "statuses"');
    ...
    if (settledDeployment.status === "fulfilled") {
      ...
      core.warning('May need "GITHUB_TOKEN" with "write" permission to "deployments"');
  ...
  try {
    ...
    await commitStatus?.update({
      ...
      description: readyState === "READY" ? `Ready for ${deploymentEnvironmentName}` : "Failed",
    });
